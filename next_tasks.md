Add a GM panel that shows a list of the characters added, and allows the GM to add new characters, edit existing ones, or remove them. Add a load/save for JSON to this page, so the GM can load/save each character to a file between sessions. A character will have basic name, description, race, class, health, mana, gold, skills, and inventory. 
...
Add a 'object type' to the world. This should identify what type of object something is. That way there can be multiple instances (objects) of the same type. For example, a 'blue key' could be type 101. Multiple blue keys could exist, but they would all be type 101. A lock would be opened by a key of type 101. Another type 102 for 'red key' would then only work on locks that were assigned it.

Add a 'blue key' and 'red key' item to the world and place it randomly outside our starting rooms, along the edge of our generated world. The key is a small item that can be picked up and dropped. The key should have a description, and should be able to be used to open specific doors, as its type will be 101. 

Allow the GM to see a master list of item types, and drag any one of them into the world to create an instance. Allow the GM to edit the properties of the item type, and save them to a json file. Allow the GM to add or delete the item types.

Allow the GM to see a master list of creature types, and drag any one of them into the world to create an instance. Allow the GM to edit the properties of the creature type, and save them to a json file. Allow the GM to add or delete the creature types.

Allow the GM to see a master list of character types, and drag any one of them into the world to create an instance. Allow the GM to edit the properties of the character type, and save them to a json file. Allow the GM to add or delete the character types.
